...
@section('content')
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <div>
                ...
                <div class="text-muted small">Escolha os contatos e telefones para enviar. Clique nos itens abaixo para selecioná-los/desselecioná-los.</div>
            </div>
            <div class="text-right small">Contatos selecionados: <span id="selectedContatosCount">0</span>/<span id="totalContatosCount">0</span></div>
        </div>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('campanhas.crud.store') }}" id="campanhaForm">
                    ...
                    <div id="contatosList" class="mb-2">
                        @foreach($contatos as $contato)
                            <div class="d-inline-block m-0 p-0">
                                <button type="button" class="btn btn-light btn-sm contato-item m-1" data-id="{{ $contato->id }}" aria-pressed="false">{{ $contato->name }}</button>
                                <input type="checkbox" name="contatos[]" value="{{ $contato->id }}" id="contato_checkbox_{{ $contato->id }}" class="contato-checkbox d-none">
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="mb-3">
                    <label class="form-label">Telefones <small class="text-muted">(clique para selecionar)</small></label>
                    <div id="telefonesList" class="mb-2">
                        @foreach($telefones as $telefone)
                            <div class="d-inline-block m-0 p-0">
                                <button type="button" class="btn btn-light btn-sm telefone-item m-1" data-id="{{ $telefone->id }}" aria-pressed="false">{{ $telefone->phone_number }}</button>
                                <input type="checkbox" name="telefones[]" value="{{ $telefone->id }}" id="telefone_checkbox_{{ $telefone->id }}" class="telefone-checkbox d-none">
                            </div>
                        @endforeach
                    </div>
                </div>
    ...
    @push('scripts')
    <style>
        .contato-item:focus, .telefone-item:focus { outline: 3px solid rgba(13,110,253,0.25); outline-offset: 2px; }
        .contato-item.contato-selected { background-color: #0d6efd; color: #fff; border-color: #0d6efd; }
        .telefone-item.telefone-selected { background-color: #198754; color: #fff; border-color: #198754; }
    </style>
        ...
        (function(){
            ...
            contatoButtons.forEach(btn => {
                ...
                btn.addEventListener('click', () => {
                    ...
                    if(!cb) { console.log('no cb for', id); return; }
