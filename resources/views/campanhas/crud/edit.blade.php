        ...
        <div class="card-body">
            <form method="POST" action="{{ route('campanhas.crud.update', $campanha) }}" id="campanhaForm">
                ...
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <div class="mb-3">
                            ...
                            <div id="imagensContainer" class="d-flex flex-wrap gap-2 p-3 bg-light rounded" style="min-height: 120px;">
                                <p class="text-muted w-100" id="semImagemMsg">Nenhuma imagem selecionada</p>
                            </div>

                            {{-- Input oculto para enviar a imagem selecionada --}}
                            <input type="hidden" name="img_campanha" id="img_campanha_input" value="{{ $campanha->img_campanha }}">
                        </div>
                    </div>
                    <div class="col-md-8">
                ...
                <div class="form-group">
                    ...
                    <textarea name="mensagem" class="form-control" rows="3" placeholder="Digite a mensagem da campanha (aceita emojis)">{{ old('mensagem', $campanha->mensagem) }}</textarea>
                </div>
            ...
            function carregarImagens() {
                fetch('{{ route("imagens.list") }}')
                    ...
                    .then(data => {
                        ...
                        const imagemAtualId = parseInt(imgCampanhaInput.value || 0);
                        data.imagens.forEach(img => {
                            imagensSelecionadas.set(img.id, {
                                id: img.id,
                                url: img.url,
                                caminho: img.caminho_imagem,
                                selected: img.id === imagemAtualId
                            });
                        });
                        renderizarImagens();
                    })
